...
def generate_rotating_nerf(neural_radiance_field, target_cameras, renderer_grid, n_frames = 50, device=torch.device("cpu")):
    ...
    # Stack frames along a new frame dimension to produce a tensor of
    # shape (n_frames, H, W, 3). Using cat would concatenate the image
    # rows and produce a single tall image instead of a sequence.
    if len(frames) == 0:
        raise RuntimeError("No frames were rendered - check the renderer and camera setup.")
    frames_stacked = torch.stack(frames, dim=0)
    # Debug: print shape to help diagnose empty-output issues
